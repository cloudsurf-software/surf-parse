...
/// The one-snippet-per-kind table lives in the crate now
/// (`spec_registry::SNIPPETS`, 0.27) so a player can show every block with
/// at least one example; this suite keeps pinning it against the registry.
const SNIPPETS: &[(&str, &str)] = surf_parse::spec_registry::SNIPPETS;
